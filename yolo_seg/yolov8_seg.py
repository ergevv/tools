...
# # 指定只在第0号CPU核心上运行
# os.sched_setaffinity(0, {5})  # 第二个参数是允许运行的CPU核心编号集合

# print(f"当前进程绑定到: {os.sched_getaffinity(0)}")
...
IMG_SIZE = (64, 64)  # (width, height), such as (1280, 736)
...
def post_process(input_data):
    ...
    classes_conf = np.concatenate(classes_conf)  #只保留概率最大的
    scores = np.concatenate(scores)
    seg_part = np.concatenate(seg_part)
    classes = np.zeros_like(scores)

    real_keeps = np.argmax(classes_conf, axis=0)    
    ...
    # agnostic = 0
    # max_wh = 7680
    # c = classes * (0 if agnostic else max_wh)  #实现类间独立抑制
    ...
    nscores.append(classes_conf[real_keeps])
...
def draw(image, boxes, scores, classes):
    for box, score, cl in zip(boxes, scores, classes):
        ...
        print("%s @ (%d %d %d %d) %.3f" % (CLASSES[int(cl.item())], top, left, right, bottom, score))
        cv2.rectangle(image, (top, left), (right, bottom), (255, 0, 0), 2)
        cv2.putText(image, '{0} {1:.2f}'.format(CLASSES[int(cl.item())], score.item()),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device_id', type=str, default=[0,1,2], help='device id')
    ...
    parser.add_argument('--img_folder', type=str, default='/home/datatset', help='img folder path')
    ...
    for i in range(len(img_list)):
        ...
        elapsed_time = (end_time - start_time)/iters  # 计算经过的时间
        print(f"推理时间: {infer_time/iters} 秒")
