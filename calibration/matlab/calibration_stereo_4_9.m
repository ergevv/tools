...
%% 单目
leftImageFiles = dir(fullfile(leftImageDir, '*.*g')); 
rightImageFiles = dir(fullfile(rightImageDir, '*.*g'));
...
% leftImgPath = '/home/erge/work/Aceii/calibration/photos/robot01/image/leftImage/4916073408.png'
leftI = imread(leftImgPath);

rightImgPath = fullfile(right_stereo_path, right_stereo_name{1});
% rightImgPath = '/home/erge/work/Aceii/calibration/photos/robot01/image/rightImage/4916073408.png'
rightI = imread(rightImgPath);
...
% %% 
% % 给定的3D点（在左相机坐标系下）
% P_left_cam = [100; 100; 23000];  % 3D point in left camera coordinates
% 
% % 获取左相机内参矩阵
% K1 = stereoParams.CameraParameters1.IntrinsicMatrix;  % 3x3
% K2 = stereoParams.CameraParameters2.IntrinsicMatrix;  % 3x3
% 
% % 右相机相对于左相机的旋转和平移
% R = stereoParams.RotationOfCamera2;  % 3x3
% T = stereoParams.TranslationOfCamera2;  % 3x1
% 
% %% 1. 投影到左图像
% p_left_homogeneous = K1 * P_left_cam;
% p_left = p_left_homogeneous(1:2) / p_left_homogeneous(3);  % [u, v]
% u_left = p_left(1);
% v_left = p_left(2);
% 
% %% 2. 将3D点转换到右相机坐标系
% P_right_cam = R' * (P_left_cam - T);  % 注意：R 是左到右的旋转，所以右到左是 R'
% % 或者更直接：P_right = R' * P_left - R' * T
% 
% % 投影到右图像
% p_right_homogeneous = K2 * P_right_cam;
% p_right = p_right_homogeneous(1:2) / p_right_homogeneous(3);
% u_right = p_right(1);
% v_right = p_right(2);
% 
% %% 3. 加载原始图像（非校正图像）
% % 注意：我们希望将点绘制在原始图像上，而不是校正后的图像
% leftImgPath = '/home/erge/work/Aceii/calibration/photos/baseline300/2025092802/image/leftImage/1759111519757124.png';
% rightImgPath = '/home/erge/work/Aceii/calibration/photos/baseline300/2025092802/image/rightImage/1759111519757124.png';
% 
% I_left = imread(leftImgPath);
% I_right = imread(rightImgPath);
% 
% %% 4. 显示图像并绘制投影点
% figure;
% subplot(1, 2, 1);
% imshow(I_left);
% hold on;
% plot(u_left, v_left, 'r+', 'MarkerSize', 15, 'LineWidth', 2);
% text(u_left, v_left, ' Projected 3D Point', 'Color', 'yellow', 'FontSize', 10, 'BackgroundColor', 'black');
% title('Left Image - Projected Point');
% hold off;
% 
% subplot(1, 2, 2);
% imshow(I_right);
% hold on;
% plot(u_right, v_right, 'b+', 'MarkerSize', 15, 'LineWidth', 2);
% text(u_right, v_right, ' Projected 3D Point', 'Color', 'yellow', 'FontSize', 10, 'BackgroundColor', 'black');
% title('Right Image - Projected Point');
% hold off;
% 
% 
% 
